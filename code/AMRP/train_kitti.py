...
def train(model, net, merge='max', vote=0, check=True, augm=True, load=True, nb_epoch=100):
    if(net != None):
        ...
        if(merge == 'maj'):
            hc.vote_value = vote
            model.compile(loss=Vote, optimizer=sgd, metrics=["accuracy"]) #metrics={'ofuse': ofuse_pixel_error})
        else:
