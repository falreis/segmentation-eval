...
def ReshapePermute(bn, name):
    on = Reshape((hc.n_classes, hc.data_shape), input_shape=(hc.n_classes, hc.height, hc.width))(bn)
    on = Permute((2, 1), name=name)(on)
    return on

###########
### HED ###
###########
def model_slo(merge_name, side_output=False):
    ...
    #reshape
    ofuse = ReshapePermute(fuse, 'ofuse')

    if(side_output):
        o1 = ReshapePermute(b1, 'o1')
        o2 = ReshapePermute(b1, 'o2')
        o3 = ReshapePermute(b1, 'o3')
        o4 = ReshapePermute(b1, 'o4')
        o5 = ReshapePermute(b1, 'o5')
...
### FULL ###
############
def model_alo(merge_name, side_output=False):
    inputs = Input((3, hc.height, hc.width))
    ...
    if(side_output):
        o1 = ReshapePermute(b12, 'o1')
        o2 = ReshapePermute(b22, 'o2')
        o3 = ReshapePermute(b33, 'o3')
        o4 = ReshapePermute(b43, 'o4')
        o5 = ReshapePermute(b53, 'o5')
        model = Model(inputs=inputs, outputs=[ofuse, o1, o2, o3, o4, o5])
    else: 
        model = Model(inputs=inputs, outputs=ofuse)
    #print(model.summary())
