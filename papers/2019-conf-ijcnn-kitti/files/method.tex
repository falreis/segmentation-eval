...
\small
\begin{align}
\mathcal{H}_{SLO}=\{\mathcal{H}_1,...,\mathcal{H}_m|& m\in[1,S]~\text{and}~\textcolor{red}{\mathcal{H}_m \in\{S^{(1)},S^{(2)}\}}\}\\
\mathcal{H}_{ALO}=\{\mathcal{H}_1,...,\mathcal{H}_m|& m\in[1,L-1]~\text{and}~\nonumber\\&\textcolor{red}{\mathcal{H}_m=C_l}~\forall l \in \{1,..,L-1\}\}
\end{align}
\normalsize
...
%\begin{figure}
 % \caption{\color{red}Side ouputs. Image must be improved!}
  %\centering
 % \includegraphics[width=1.\columnwidth]{figures/falreis/side_outputs_method.pn%g}
